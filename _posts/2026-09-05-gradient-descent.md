...
$$
L(w_1,w_2)=(w_1-1)^2+(w_2-1)^2
$$

입니다. 최솟값은 초록 점 $$(1,1)$$입니다.  
시작점은 $$(-1.2,\ 2.2)$$, 학습률은 $$\eta=0.15$$, 20걸음입니다.  
빨간 점이 금색 궤적을 남기며 바닥으로 붙습니다.
...
<mark>부호를 경우마다 외울 필요 없습니다. 항상 기울기를 뺍니다.</mark>
...
$$
\frac{\partial L}{\partial w_1}=2(-1.2-1)=-4.4
$$

$$
\frac{\partial L}{\partial w_2}=2(2.2-1)=2.4
$$

$$\eta=0.15$$로 한 걸음:

$$
w_1\leftarrow -1.2-0.15\cdot(-4.4)=-1.2+0.66=-0.54
$$

$$
w_2\leftarrow 2.2-0.15\cdot 2.4=2.2-0.36=1.84
$$

$$w_1$$은 커지고 $$w_2$$는 작아져, 둘 다 $$(1,1)$$ 쪽으로 갑니다.
...
시작점 $$(-1.2,\ 2.2)$$, $$\eta=1.05$$의 첫 걸음:

$$
w_1\leftarrow -1.2-1.05\cdot(-4.4)=-1.2+4.62=3.42
$$

$$
w_2\leftarrow 2.2-1.05\cdot 2.4=2.2-2.52=-0.32
$$

$$(1,1)$$을 지나 $$w_1=3.42$$까지 가 버립니다.
...
시작점 $$(-1.0,\ 2.0)$$, $$\eta=0.12$$이면

$$
\frac{\partial L}{\partial w_1}=2(-1-1)=-4
$$

$$
\frac{\partial L}{\partial w_2}=16(2-1)=16
$$

$$
w_1\leftarrow -1.0-0.12\cdot(-4)=-1.0+0.48=-0.52
$$

$$
w_2\leftarrow 2.0-0.12\cdot 16=2.0-1.92=0.08
$$

$$w_1$$은 $$1$$까지 아직 멀고, $$w_2$$는 $$1$$을 지나 $$0.08$$로 떨어집니다.  
같은 $$\eta$$인데 축마다 걸음이 다릅니다.
...
탭 **3. 좁은 골짜기**에서 $$w_2$$가 진동하는 궤적을 보세요.  
입력 표준화, 적응 학습률을 쓰는 이유가 여기 있습니다.
...
실무 기본값은 미니배치입니다.  
에폭 한 번은 학습 데이터를 한 바퀴 도는 것입니다. 대략 $$N/B$$ 걸음입니다.
...
손실이 큰데 기울기만 0이면 멈춘 것이지 끝난 것이 아닙니다.
...
**2.** $$w^{(2)}_1=0.4$$, 기울기 $$-0.267$$, $$\eta=0.1$$이면 새 값은?
