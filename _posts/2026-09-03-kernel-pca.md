...
categories: [AI, 머신러닝]
tags: [차원축소, PCA, 커널PCA, RBF, 핵트릭, 빅분기]
math: true
---
선형 PCA는 분산이 큰 **직선** 축만 찾습니다.  
초승달처럼 휘어진 데이터는 축을 돌려도 안 펴집니다.  
Kernel PCA는 점을 더 높은 차원의 $$\phi(x)$$로 보낸 뒤, 그 공간에서 PCA를 합니다.  
$$\phi(x)$$ 자체는 안 만들고, 점 사이 핵 값 $$k(x,x')$$만 씁니다.

<blockquote class="prompt-info">
  <p>Kernel PCA = 핵 행렬을 가운데 맞춘 뒤 고유분해하는 PCA. 선형 핵이면 보통 PCA와 같습니다. sklearn은 <code>sklearn.decomposition.KernelPCA</code>입니다. 차트는 <code>https://jyjnote.github.io/assets/plotly/kernel-pca-charts.html</code> 입니다.</p>
</blockquote>
...
초승달 두 개가 껴 있으면, 긴 방향은 달의 길이이고 두 반을 가르는 방향이 아닙니다.  
축을 돌려도 달은 달입니다.
...
탭 **2. 선형 PCA**로 넘기면 축만 바뀌고, 두 반이 여전히 겹칩니다.
...
k_{\mathrm{rbf}}(x,x') &= \exp\big(-\gamma\lVert x-x'\rVert^2\big)
\end{aligned}
$$

- $$k_{\mathrm{lin}}$$: 선형. 그냥 PCA
- $$\gamma$$: 스케일. 폴리에서는 내적에 곱하는 수, RBF에서는 거리 민감도
- $$c$$: 폴리 핵의 상수항. 보통 1
- $$d$$: 폴리 차수. $$d=2$$면 이차식까지 포함
- $$\lVert x-x'\rVert^2$$: 두 점 사이 거리의 제곱
- RBF: 거리가 0이면 $$k=1$$, 멀수록 $$k$$가 0에 가까움

선형 핵을 쓰면 Kernel PCA는 (스케일만 다를 수 있는) PCA와 같습니다.
...
`\big(` 는 괄호가 있으니 깨지지 않습니다. `\{` 는 쓰지 않습니다.

## 핵 행렬

학습 점 $$n$$개가 있으면 $$n \times n$$ 핵 행렬을 만듭니다.
...
- $$\bar{K}_{i\cdot}$$: i번 행의 평균. i번이 다른 점들과 맺는 핵의 평균
...
- $$\tilde{k}(x_i,x)$$: 새 점과 학습 점 i의 핵 값을, 학습 평균 기준으로 가운데 맞춘 값
- 합: 모든 학습 점이 새 점에 기여
- $$\lambda_j=0$$이면 나눌 수 없음. 그 축은 버림

새 점을 쓰려면 학습 점과 핵 값을 **다시** 계산해야 합니다.  
...
$$
\bar{K}_{A\cdot}=\frac{1.7358}{3}\approx 0.5786,\quad
...
\tilde{K}_{AA}
=
1-0.5786-0.5786+0.5269
\approx 0.3697
$$

같은 방식으로 나머지 원소를 계산하면

$$
\tilde{K}
\approx
\begin{pmatrix}
0.3697 & -0.1849 & -0.1849 \\
-0.1849 & 0.5248 & -0.3399 \\
-0.1849 & -0.3399 & 0.5248
\end{pmatrix}
$$
...
$$
\lambda_1\approx 0.865,\quad
\lambda_2\approx 0.555,\quad
\lambda_3\approx 0
$$
...
탭 **3. RBF Kernel PCA** (감마=12)에서 두 반이 거의 갈라집니다.
...
- `gamma`: RBF·폴리의 $$\gamma$$. `None`이면 $$1/p$$
- `degree`, `coef0`: 폴리의 $$d$$, $$c$$
...
| 새 점 | $$v^\top(x-\bar{x})$$ | 학습 점과 핵을 다시 계산 |
| 역변환 | $$\bar{x}+V_k z$$ | 근사만 가능 |
| 비용 | $$p$$에 비례하는 편이 흔함 | $$n^2$$ 핵 + $$n^3$$ 고유분해 |
| 초모수 | 성분 수, 표준화 | 핵 종류, $$\gamma$$, $$d$$ |

둘 다 정답 라벨을 보지 않습니다.  
Kernel PCA가 반을 잘 가르는 것은 데이터가 핵과 맞을 때뿐입니다.
...
**9. $$n$$이 크면 비싸다**  
핵 행렬 $$n\times n$$을 못 올리면 Nyström 근사 등을 씁니다.
...
- 사람 수가 매우 클 때
- 반을 가르는 축이 목표면(그때는 커널 SVM·LDA 계열을 검토)
- 새 점을 아주 빠르게 올려야 하고 학습 점을 못 들고 있을 때

## 잘 놓치는 핵심

- Kernel PCA = $$\phi$$ 공간의 PCA를 핵으로 계산
- $$K_{ij}=k(x_i,x_j)$$
- $$\tilde{K}$$를 고유분해
...
**2.** RBF 핵 $$e^{-\gamma\lVert x-x'\rVert^2}$$에서 두 점이 같으면?
- ① 0
- ② 1
- ③ $$\gamma$$
- ④ $$n$$
...
**4.** $$\tilde{K}=K-\mathbf{1}K-K\mathbf{1}+\mathbf{1}K\mathbf{1}$$의 역할은?
- ① 라벨을 맞춘다
- ② $$\phi$$ 공간에서 평균을 뺀 것과 같은 효과
- ③ $$\gamma$$를 고른다
- ④ 변수를 삭제한다
<details>
<summary>정답</summary>
②  
선형 PCA의 평균 빼기에 해당합니다.
</details>

**5.** 사람 $$n=5$$일 때 의미 있는 Kernel PCA 축의 상한은?
- ① 5
- ② 4
- ③ 무한
- ④ $$\gamma$$
...
<summary>정답</summary>
②  
차트 탭 4의 감마 0.5가 이 방향입니다.
</details>

**7.** 새 점 $$x$$의 Kernel PCA 좌표를 구하려면?
- ① 학습 점과의 핵 값이 필요하다
- ② $$v^\top x$$만 있으면 된다
- ③ 고유값은 필요 없다
- ④ 반드시 역변환부터 한다
...
<summary>정답</summary>
①  
$$z_j(x)=\lambda_j^{-1/2}\sum_i\alpha_{ij}\tilde{k}(x_i,x)$$입니다.
</details>

**8.** 위 3점 예에서 $$k(B,C)$$, $$\gamma=1$$은?
- ① $$e^{0}=1$$
- ② $$e^{-1}\approx 0.37$$
- ③ $$e^{-2}\approx 0.14$$
- ④ 0
...
<summary>정답</summary>
③  
$$\lVert B-C\rVert^2=2$$이므로 $$e^{-2}$$입니다.
</details>

**9.** Kernel PCA의 계산 부담으로 가장 맞는 것은?
- ① 항상 $$p$$만 커지면 된다
- ② 핵 행렬이 $$n\times n$$이라 $$n$$이 커지면 부담이 급증
- ③ 고유분해가 필요 없다
- ④ 표준화가 금지된다
<details>
<summary>정답</summary>
...
**10.** 초승달 두 반이 껴 있을 때 선형 PCA 1축이 반을 잘 가르나?
- ① 항상 그렇다
- ② 아니다. 직선 축만으로는 휘어진 구조가 안 펴질 수 있다
- ③ Kernel PCA는 항상 라벨을 쓰므로 가른다
- ④ $$\lambda<0$$이면 가른다
<details>
<summary>정답</summary>
②  
차트 탭 2와 탭 3을 비교하면 됩니다.
</details>

다음 글은 정답을 보고 축을 고르는 **LDA**입니다.  
분산이 큰 방향, 핵으로 편 방향, 반을 가르는 방향을 이어서 비교합니다.
